...
로컬 Codex ChatGPT 로그인으로 이미지를 빠르게 생성하는 도구입니다.
...
대신 즉시 실패합니다. `auto`는 사용자 의도를 놓칠 수 있는 폴백을
...
때가 많습니다. Codex 백엔드를 직접 호출하는 `codex` 경로는 빠르고
참조 이미지, 화면비, 다중 이미지 생성을 지원합니다. `codex-cli` 경로는 더
느리지만 비공개 HTTP 경로가 실패할 때 대체 수단으로 쓸 수 있습니다.
...
| `auto` | 비공개 경로 먼저, CLI 폴백 | 비공개 경로에서 가능 | 비공개 경로에서 가능 | 비공개 경로에서 가능 | `--` 플래그가 사라질 수 있으면 폴백 거부 |
...
제외), `--number > 1` 중 하나라도 설정됐을 때 `codex-cli` 폴백을
...
에러 응답은 JSON이며 `error`, `code`를 담습니다. 생성 도중 실패한 경우에는
...
응답에는 절대 경로와 함께 출력 루트 기준 POSIX 스타일 상대 경로가
...
훅은 자신의 위치를 기준으로 저장소 루트를 찾으므로 셸 파이프라인이나
다른 도구에서 호출하기 쉽습니다.
